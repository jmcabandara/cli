...
func (cmd *UpdateUserProvidedService) MetaData() command_registry.CommandMetadata {
	...
	fs["p"] = &cliFlags.StringFlag{Name: "p", Usage: T("Credentials exposed in the VCAP_SERVICES environment variable for bound applications")}
	fs["l"] = &cliFlags.StringFlag{Name: "l", Usage: T("URL to which logs for bound applications will be streamed")}
	fs["r"] = &cliFlags.StringFlag{Name: "r", Usage: T("URL to which requests for bound routes will be forwarded. Scheme for this URL must be https")}
