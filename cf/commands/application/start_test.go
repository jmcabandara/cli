...
import (
	...
	testapplications "github.com/cloudfoundry/cli/cf/api/applications/fakes"
...
var _ = Describe("Start", func() {
	var (
		...
		origAppCmd command_registry.Command
		appCmd     *testcmd.FakeAppDisplayer
	...
	BeforeEach(func() {
		...
		appCmd = &testcmd.FakeAppDisplayer{}
		origAppCmd = command_registry.Commands.FindCommand(appCmd.MetaData().Name)
		Expect(origAppCmd).NotTo(BeNil(), "expected this to be here from test pollution. Remove this!")
		command_registry.Register(appCmd)
	...
	AfterEach(func() {
		command_registry.Commands.RemoveCommand(appCmd.MetaData().Name)
		command_registry.Register(origAppCmd)
	})
